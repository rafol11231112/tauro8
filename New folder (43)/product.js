    ...
    if (product) {
        ...
        buyButton.onclick = async function() {
            ...
            try {
                ...
                const response = await fetch('http://localhost:8080/send-purchase-email', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (result.success) {
                    alert('Purchase successful! Check your email and customer panel for your item.');
                } else {
                    throw new Error(result.message);
                }
            } catch (error) {
                console.error('Error sending email:', error);
                alert('Purchase successful but failed to send email. Please check your customer panel for your item.');
            } finally {
